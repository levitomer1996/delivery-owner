...
function DashboardContent() {
  const { pageState, setPage } = useContext(PageContext);
  const { authState } = useContext(AuthContext);
  const { isLogged } = authState;
  const [error, isInitFinished, tokenSignin] = useAutoSignin();
      ...
      <Box sx={{ display: "flex" }}>
        ...
        <AppBar position="absolute" open={open}>
          ...
          >
            ...
            <Typography
              ...
              sx={{ flexGrow: 1 }}
            >
              {authState.isLogged ? `Welcome ${authState.user.f_name}` : null}
            </Typography>
        ...
        >
          ...
          <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
            {renderDashboard(pageState.page)}
            <Copyright sx={{ pt: 4 }} />
          </Container>
        </Box>
      </Box>
